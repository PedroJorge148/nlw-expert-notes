...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    const isSpeechRcognitionAPIAvailable =
      'SpeechRecognition' in window || 'webkitSpeechRecognition' in window

    if (!isSpeechRcognitionAPIAvailable) {
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-0 flex flex-col overflow-hidden bg-slate-700 outline-none md:inset-auto md:left-1/2 md:top-1/2 md:h-[60vh] md:w-full md:max-w-[640px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-md">
          <Dialog.Close className="absolute right-0 top-0 bg-slate-800 p-1.5 hover:text-slate-100 focus-visible:ring-lime-400">
            <X className="size-5" />
          </Dialog.Close>
          <form className="flex flex-1 flex-col">
            ...
            {isRecording ? (
              <button
                ...
                className="flex w-full items-center justify-center gap-2 bg-slate-900 py-4 text-center text-sm text-red-400 outline-none hover:text-slate-100"
              >
